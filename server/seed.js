...
const seed = async () => {
  try {
    ...
    await Promise.all(  //Promise.all accepts an array of all data and it individually handles all of it
      users.map(async user => {
        const data = await db.User.create(user);    //creting users
        await data.save();  //saving users
      }),
    );
